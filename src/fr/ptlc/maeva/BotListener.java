...
public class BotListener implements EventListener {
	 ...
	 public void onEvent(GenericEvent event) {
			if (event instanceof MessageReceivedEvent) {
				...
				if (e.getReaction().retrieveUsers().complete().contains(e.getJDA().getSelfUser())) {
					...
					if (e.getChannel().getId().equals(save.getOptions().getCommandsChannelId())) {
						if (emoteName.equals(save.getOptions().getEmote2Name())) {
							...
							maevaQuery.editMessage(e.getUser().getAsMention() + ", une personne comp�tante vous r�pondra d�s qu'elle le pourra, puis ajoutez l'emote " + Functions.getEmoteTag(e.getJDA(), save.getOptions().getEmote3Name()) + "lorsque vous serez satisfait").queue();
							maevaQuery.delete().queueAfter(5, TimeUnit.SECONDS);
					...
					if (e.getChannel().getId().equals(server.getAntiSpamChannelId())) {
						...
						if (emoteName.equals(server.getAntiSpamEmote()) && message.getMentionedMembers().contains(e.getMember())
							...
							e.getChannel().sendMessage(e.getUser().getAsMention() + ", bienvenue � toi !").queue();
						}
					}
				}
			}
				...
				((GuildJoinEvent)event).getGuild().getDefaultChannel().sendMessage("Merci de m'avoir ajout�e sur votre serveur !\nN'h�sitez pas � envoyer `" + Save.getMaeva().getData().getServerById(((GuildJoinEvent)event).getGuild().getId()).getPrefix() + "help` pour avoir de l'aide\nEt si vous rencontrez des probl�mes contactez " + Functions.getUser(event.getJDA(), UserProfession.CREATOR).getAsMention()).queue();
